    ...
    async def validate_container_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not result["success"]:
                remedy = "Create and start it with: exegol install web && exegol start aida (or set another container in AIDA Settings)."
                try:
                    others = await self.discover_containers()
                    if others:
                        names = ", ".join(c["name"] for c in others[:5])
                        remedy = f"Use an existing container in AIDA Settings (e.g. {names}) or create default: exegol install web && exegol start aida"
                except Exception:
                    pass
                return {
                    "success": False,
    ...
    async def execute_container_command(self, container_name: str, command: str) -> Dict[str, Any]:
        ...
        if not validation["success"]:
            ...
            if validation.get("remedy"):
                details = f"{details}\n\n→ {validation['remedy']}" if details else validation["remedy"]
            return {
                "success": False,
