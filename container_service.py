...
class ContainerService:
    ...
    async def validate_container_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not result["success"]:
                # User-friendly message with remedy (Docker stderr is e.g. "Error response from daemon: No such container: exegol-aida")
                remedy = f"Create and start it with: exegol install web && exegol start aida   (or set another container in Settings)"
                others = await self.discover_containers()
                if others:
                    names = ", ".join(c["name"] for c in others[:5])
                    remedy = f"Use an existing container in Settings (e.g. {names}) or create default: exegol install web && exegol start aida"
                return {
                    "success": False,
                    "error": f"Container '{self.current_container}' not found",
    ...
    async def execute_container_command(
        ...
        if not validation["success"]:
            # If configured container missing but exactly one Exegol exists, use it
            if "not found" in validation.get("error", "").lower() or "no such container" in (validation.get("details") or "").lower():
                available = await self.discover_containers()
                if len(available) == 1:
                    self.current_container = available[0]["name"]
                    validation = await self.validate_container_status()
                    if validation["success"]:
                        # Fall through to execution below
                        pass
                    else:
                        self.current_container = settings.DEFAULT_CONTAINER_NAME  # revert
            if not validation["success"]:
                stderr = validation.get("details", validation["error"])
                if validation.get("remedy"):
                    stderr = f"{stderr}\n\n→ {validation['remedy']}" if stderr else validation["remedy"]
                return {
                    "success": False,
